...
import { GetTasksErrorSchema, GetTasksParamsSchema } from './get-tasks';
...
describe('GetTasksParamsSchema', () => {
  it('accepts an empty object (archived is optional)', () => {
    expect(GetTasksParamsSchema.parse({})).toEqual({});
  });

  it.each([true, false])('accepts archived %s', (archived) => {
    expect(GetTasksParamsSchema.parse({ archived })).toEqual({ archived });
  });

  it('strips unexpected props', () => {
    const result = GetTasksParamsSchema.parse({
      archived: true,
      unexpected: 'bar',
      another: 'baz',
    });
  ...
  describe('archived', () => {
    it.prop({ v: $nonBoolean })('rejects a non-boolean archived', ({ v }) => {
      const result = GetTasksParamsSchema.safeParse({ archived: v });
...
describe('GetTasksErrorSchema', () => {
  describe('common error codes', () => {
    it('accepts functions/invalid-argument/schema', () => {
      ...
      expect(() => GetTasksErrorSchema.parse(err)).not.toThrow();
    });
  ...
  it('rejects an unknown error code', () => {
    ...
    expect(() => GetTasksErrorSchema.parse(err)).toThrow();
  });
});
